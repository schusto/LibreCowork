...
import type { ElicitationRequest, ElicitationPropertySchema } from 'librechat-data-provider';
  ...
  const renderField = (key: string, property: ElicitationPropertySchema) => {
    ...
    if (property.type === 'string') {
      if (property.minLength)
        validationRules.minLength = { value: property.minLength, message: `Minimum length is ${property.minLength}` };
      if (property.maxLength)
        validationRules.maxLength = { value: property.maxLength, message: `Maximum length is ${property.maxLength}` };
      if (property.format === 'email')
        validationRules.pattern = { value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, message: 'Please enter a valid email address' };
    }
    if (property.type === 'number' || property.type === 'integer') {
      if (property.minimum !== undefined)
        validationRules.min = { value: property.minimum, message: `Minimum value is ${property.minimum}` };
      if (property.maximum !== undefined)
        validationRules.max = { value: property.maximum, message: `Maximum value is ${property.maximum}` };
    }
    ...
    const renderInput = () => {
      ...
      if (property.type === 'number' || property.type === 'integer') {
        return (
          <input
            ...
            {...register(key, { ...validationRules, valueAsNumber: true })}
            className="w-full rounded border border-border-medium bg-surface-primary px-3 py-2 text-text-primary focus:border-border-heavy focus:outline-none"
          />
        );
      }
      ...
      else if (property.format === 'date') inputType = 'date';
      else if (property.format === 'date-time') inputType = 'datetime-local';
    ...
    return (
      <div key={key} className="mb-4">
        ...
        {property.description && (
          <p className="mb-2 whitespace-pre-line text-xs text-text-secondary">{property.description}</p>
        )}
        {renderInput()}
        {errors[key] && (
    ...
    <div className="mx-auto max-w-2xl rounded-lg border border-border-medium bg-surface-primary p-6 shadow-sm">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="flex justify-end space-x-3 pt-4">
          <Button type="button" variant="outline" onClick={onCancel} disabled={isSubmitting} className="px-4 py-2">
            {localize('com_ui_cancel')}
          </Button>
          <Button type="button" variant="outline" onClick={onDecline} disabled={isSubmitting} className="px-4 py-2">
