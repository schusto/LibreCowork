...
const ResumableAgentController = async (req, res, next, initializeClient, addTitle) => {
  ...
  try {
    ...
    // MCP Elicitation: forward elicitation-created events for this user's tool
    // calls onto the resumable event stream so the frontend can render the
    // approval form. finishTerminalJob() always calls job.abortController.abort()
    // on every terminal path (success, error, or true abort), so hooking cleanup
    // to that signal removes the listener exactly once regardless of how this
    // generation ends.
      ...
      if (eventData.userId === userId) {
        ...
        if (elicitationState) {
          GenerationJobManager.emitChunk(
            ...
            logger.error('[ResumableAgentController] Error emitting elicitation event', err);
          });
        }
      }
    };
    ...
    job.abortController.signal.addEventListener('abort', removeElicitationListener, {
      once: true,
    });
