...
class AgentClient extends BaseClient {
  ...
  async buildMessages(messages, parentMessageId, _buildOptions, opts) {
    ...
    const allAgents = [
      { agent: injectConversationId(injectTodoInstructions(normalizeInstructions(this.options.agent))), agentId: this.options.agent.id },
      ...(this.agentConfigs?.size > 0
        ? Array.from(this.agentConfigs.entries()).map(([agentId, agent]) => ({
            agent: injectConversationId(injectTodoInstructions(normalizeInstructions(agent))),
            agentId,
          }))
        : []),
    ];
  ...
  async chatCompletion({ payload, userMCPAuthMap, abortController = null }) {
    ...
    try {
      ...
      // [cowork] Guard: Qwen3's Jinja template requires the first non-system turn to be a
      // user turn. After summary boundary processing, the assistant message's post-summary
      // text (content after the summary block) yields a leading AIMessage in initialMessages,
      // producing [SystemMessage, AIMessage(post-summary), HumanMessage(user)] — which Qwen3
      // rejects with "No user query found in messages." (same error as having no user at all).
      // Also handles the degenerate case where the boundary consumed the user turn entirely.
      if (initialMessages.length === 0 || initialMessages[0]._getType() !== 'human') {
        const firstHumanIdx = initialMessages.findIndex((m) => m._getType() === 'human');
        if (firstHumanIdx === -1) {
          // No human message at all — re-append current user turn from payload tail.
          const lastPayloadMsg = payload[payload.length - 1];
          if (lastPayloadMsg?.role === 'user') {
            logger.warn(
              '[AgentClient] No user message in initialMessages after formatAgentMessages — re-appending current user turn from payload tail',
            );
            initialMessages = [...initialMessages, formatMessage({ message: lastPayloadMsg, langChain: true })];
          }
        } else {
          // Leading AIMessage(s) before first HumanMessage: the summary boundary left the
          // assistant's post-summary text as the first message. Re-insert the user message
          // that immediately preceded the summary boundary so the sequence starts with a
          // user turn and the post-summary context is preserved:
          //   [HumanMessage(user_N), AIMessage(post-summary), ..., HumanMessage(user_current)]
          // Find the boundary message in payload (the assistant message containing the summary
          // block) and take the user message at boundaryIndex - 1.
          const boundaryIdx = payload.findIndex(
            (m) => Array.isArray(m.content) && m.content.some((p) => p?.type === 'summary'),
          );
          const precedingUserPayload = boundaryIdx > 0 ? payload[boundaryIdx - 1] : null;
          if (precedingUserPayload?.role === 'user') {
            logger.warn(
              `[AgentClient] initialMessages[0] is ${initialMessages[0]._getType()} (not human) — prepending boundary user turn (payload[${boundaryIdx - 1}]) to satisfy Qwen3 chat template`,
            );
            initialMessages = [
              formatMessage({ message: precedingUserPayload, langChain: true }),
              ...initialMessages,
            ];
          } else {
            // Fallback: no suitable preceding user message found — slice from first human.
            logger.warn(
              `[AgentClient] initialMessages[0] is ${initialMessages[0]._getType()} (not human), no preceding user at payload[${boundaryIdx - 1}] — slicing to first HumanMessage`,
            );
            initialMessages = initialMessages.slice(firstHumanIdx);
          }
        }
      }
