...
const initializeClient = async ({ req, res, signal, endpointOption }) => {
  ...
  const loadAgentById = async (agentId) => {
    ...
    try {
      ...
      if (!agent) {
        logger.warn(`[loadAgentById] SKIP ${agentId}: not found in DB`);
        skippedAgentIds.add(agentId);
        return null;
      }
      const userId = req.user?.id;
      if (!userId) {
        logger.warn(`[loadAgentById] SKIP ${agentId}: no userId on request`);
        skippedAgentIds.add(agentId);
        return null;
      }
      logger.debug(`[loadAgentById] CHECK ${agentId} name="${agent.name}" provider="${agent.provider}" model="${agent.model}" _id=${agent._id}`);
      ...
      if (!hasAccess) {
        logger.warn(
          `[loadAgentById] SKIP ${agentId} name="${agent.name}": User ${userId} lacks VIEW access`,
        );
        skippedAgentIds.add(agentId);
        return null;
      }
      ...
      if (!validation.isValid) {
        logger.warn(
          `[loadAgentById] SKIP ${agentId} name="${agent.name}": model validation failed: ${validation.error?.message}`,
        );
        skippedAgentIds.add(agentId);
        return null;
      }
      logger.debug(`[loadAgentById] OK ${agentId} name="${agent.name}" — proceeding to initializeAgent`);
